...
class DataStoreClient(object):
    ...
    def callAPI_to_storeArray2D(self, userId, pwd, interfaceId, params,inArray2D,serverId=None):
        '''
                     要素写入接口
        '''
        # 所要调用的方法名称   打印该函数名称  
        ...
        requestInfo = self.callAPI_to_storeArray2D_FileInfo(userId, pwd,\
                             interfaceId,serverId,params,method,inArray2D,None)
        
        return requestInfo
    
    def callAPI_to_storeFile(self, userId, pwd, interfaceId,params,inArray2D,inFilePaths,serverId=None,isBackstage=0,localMountPath='',serverMountPath=''):
        if(isBackstage==1):
        ...
        if(flagIsDelete==1): #删除文件索引信息
            return self.callAPI_to_storeArray2D_FileInfo(userId,pwd,interfaceId,serverId,params,method,inArray2D,None)    
        else:
            ...
            if fileSize <= 0 :
                ...
                for k in range(fileNum):
                    ...
                    if(isBackstage==1): #通过本地挂载盘写到服务端
                        ...
                        uploadUrl += '&timestamp=%s' % timestamp
                        uploadUrl += '&nonce=%s' % nonce
                        #生成sign
                        signParams = {}
                        signParams['serviceNodeId'] = self.serverId
                        signParams['fileName'] = uploadFileName
                    ...
                    if(copyResult[0] == False):
                        if(copyResult[1].__contains__(DataStoreClient.getwayFlag)): #网关错误
                            ...
                            requestInfo.errorCode = self.OTHER_ERROR
                            requestInfo.errorMessage = "upload file fail:" + copyResult[1]
                            return requestInfo;                        
                    #else:
                        #print 'upload file success:'+ fullFileName
                    
            return self.callAPI_to_storeArray2D_FileInfo(userId,pwd,interfaceId,serverId,params,method,inArray2D,httpTempNames)

        
    def callAPI_to_storeSerializedStr(self, userId, pwd, interfaceId, params,inString,serverId=None):
        '''
                    写入序列化字符串
        '''
    ...
    def callAPI_to_storeGridData(self,userId, pwd, interfaceId,params,inGridData,serverId=None):
        '''
                     写入格点数据
        '''
        requestInfo = RequestInfo()
        ...
        if((inGridData.data is not None) and (len(inGridData.data)>0)): #不是删除
            if(inGridData.pointFlag!=0): #点更新
                if((len(inGridData.lats)!=len(inGridData.lons)) or (len(inGridData.data)!=len(inGridData.lons))):
                    requestInfo.errorCode = self.OTHER_ERROR
                    requestInfo.errorMessage = 'Input data size is wrong!'
                    return requestInfo
            else:
                if(len(inGridData.data)!=(((int)(inGridData.lats[0]+0.5))*((int)(inGridData.lons[0]+0.5)))):
                    requestInfo.errorCode = self.OTHER_ERROR
                    requestInfo.errorMessage = "Input data size is wrong!"
                    return requestInfo
        
        return self.callAPI_to_storeGridDataInfo(userId,pwd,interfaceId,method,params,inGridData,serverId)

    def callAPI_to_storeBlockData(self,userId, pwd, interfaceId,params,attributes,BlockData,serverId=None):
        '''
                     写入数据块
        '''
        requestInfo = RequestInfo()
        method = "callAPI_to_storeBlockData"; #调用函数（方法）名称
        if(((interfaceId=='saveBlockData') or (interfaceId=='saveBlockData')) and (attributes is None)):
            requestInfo.errorCode = self.OTHER_ERROR
            requestInfo.errorMessage = "Input data attributes can't null!"
            return requestInfo
        
        return self.callAPI_to_storeBlockDataInfo(userId,pwd,interfaceId,method,params,attributes,BlockData,serverId)
    
    def callAPI_to_storeArray2D_FileInfo(self,userId, pwd, interfaceId, serverId,params,method,inArray2D,inFilePaths=None):
        '''
                     写入二维数据或文件信息
        '''
        requestInfo = RequestInfo()
        ...
        if((inFilePaths == None) or (len(inFilePaths) == 0)): #非文件
            ...
            if fileRow != array2DRow:
                requestInfo.errorCode = self.OTHER_ERROR
                requestInfo.errorMessage = "Input fileinfo and file number does not match !"
                return requestInfo  
        ...
        if(RetByteArraydata.__contains__(DataStoreClient.getwayFlag.encode(encoding='utf_8', errors='strict'))): #网关错误
    ...
    def getConcateUrl(self,userId, pwd, interfaceId, params, serverId, method):
        ...
        signParams['language'] = DataStoreClient.language
        signParams['clientversion'] = DataStoreClient.clientVersion
        signParams['timestamp'] = timestamp
        signParams['nonce'] = nonce
        signParams['pwd'] = pwd
        sign = self.getSign(signParams)
    ...
    def callAPI_to_storeGridDataInfo(self, userId, pwd, interfaceId, method, params, inGridData, serverId=None):
        '''
                     写入格点数据信息
        '''
    ...
    def callAPI_to_storeBlockDataInfo(self, userId, pwd, interfaceId, method, params,attributes, BlockData, serverId=None):
        '''
                     写入数据块信息
        '''
        #生成music的RequestInfo结果的对象
