...
class RequestInfo(object):
    ...
    def stringify(self):    
        return 'request = {\n errorCode:"%s"\n errorMessage:"%s"\n requestElems:"%s"\n requestParams:"%s"\n requestTime:"%s"\n responseTime:"%s"\n rowCount:"%s"\n takeTime:"%s"\n}' % (
                              self.errorCode,self.errorMessage,self.requestElems,self.requestParams,self.requestTime,self.responseTime,self.rowCount,self.takeTime)   
...
class RetArray2D(object):
    ...
    def stringify(self):    
        retData = 'data = \n{\n%s\n}\n' % (self.data)
        return '{\n%s\nrequest=%s\nrow=%s\ncol=%s\nelementNames=%s\n}' % (retData,self.request,self.row,self.col,self.elementNames)
...
class RetGridArray2D(object):
    '''
            返回格点结果
    '''
    def __init__(self, data=None, request=None, startLat=0.0, startLon=0.0, endLat=0.0, endLon=0.0, latCount=0, lonCount=0, lonStep=0.0, latStep=0.0, units='',userEleName='',lats=None,lons=None):
...
class RetGridVector2D(object):
    '''
            返回格点矢量结果
    '''
    def __init__(self, u_datas=None,v_datas=None, request=None, startLat=0.0, startLon=0.0, endLat=0.0, endLon=0.0, latCount=0, lonCount=0, lonStep=0.0, latStep=0.0,u_EleName='',v_EleName='',lats=None,lons=None):
        if request is None:
            self.request = RequestInfo() #请求信息
        else:
            self.request = request #
...
class FileInfo(object):
    ...
    def stringify(self):    
        return 'fileInfo = {\n fileName:"%s"\n savePath:"%s"\n suffix:"%s"\n size:"%s"\n fileUrl:"%s"\n imgBase64:"%s"\n attributes:"%s"\n }' % (
                              self.fileName,self.savePath,self.suffix,self.size,self.fileUrl,self.imgBase64,self.attributes)   
