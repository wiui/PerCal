...
#                                 pbRequestInfo.responseTime,pbRequestInfo.rowCount,pbRequestInfo.takeTime)
    ...
    def getRequestInfo(self, pbRetRequestInfo):
        ...
        '''
        requestInfo = MusicDataBean.RequestInfo(pbRetRequestInfo.errorCode, pbRetRequestInfo.errorMessage, \
                        pbRetRequestInfo.requestElems,pbRetRequestInfo.requestParams,pbRetRequestInfo.requestTime,\
                        pbRetRequestInfo.responseTime,pbRetRequestInfo.rowCount,pbRetRequestInfo.takeTime)
    ...
    def getDataBlock(self, pbRetDataBlock):
        ...
        '''
        retDataBlock = MusicDataBean.RetDataBlock(pbRetDataBlock.dataName,pbRetDataBlock.byteArray,pbRetDataBlock.request)
    ...
    def getGridArray2D(self, pbRetGridArray2D):
        ...
        '''
        retGridArray2D = MusicDataBean.RetGridArray2D(pbRetGridArray2D.data,pbRetGridArray2D.request)
        # 如果该结果数据存在，则将获取的数据，转换为二维数组格式
        if retGridArray2D:
    ...
    def getGridScalar2D(self, pbRetGridScalar2D):
        ...
        '''
        retGridScalar2D = MusicDataBean.RetGridScalar2D(pbRetGridScalar2D.datas,pbRetGridScalar2D.request)
        # 如果该结果数据存在，则将获取的数据，转换为二维数组格式
        if retGridScalar2D:
...
#                 retGridScalar2D.data = self.setMatrix(rows,cols,retGridScalar2D.data)
                retGridScalar2D.data = self.setMatrix(retGridScalar2D.latCount,retGridScalar2D.lonCount,retGridScalar2D.data)
    ...
    def getGridVector2D(self, pbRetGridVector2D):
        ...
        '''
        retGridVector2D = MusicDataBean.RetGridVector2D(pbRetGridVector2D.u_datas,pbRetGridVector2D.v_datas,pbRetGridVector2D.request)
        # 如果该结果数据存在，则将获取的数据，转换为二维数组格式
        if retGridVector2D:
...
#                 cols = dataLen/rows
    ...
    def setFileInfo(self,pbFileInfo):
        ...
        '''
        fileInfo = MusicDataBean.FileInfo(pbFileInfo.fileName,pbFileInfo.savePath,pbFileInfo.suffix,\
                    pbFileInfo.size,pbFileInfo.fileUrl,pbFileInfo.imgBase64,pbFileInfo.attributes)
